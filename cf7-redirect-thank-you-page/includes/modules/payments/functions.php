 ...
 */
function cf7rl_insert_payment($gateway, $mode, $amount, $form_id, $status='cf7rl-pending') {
...
function cf7rl_complete_payment($payment_id, $status, $transaction_id = '', $payer_email = '') {
	...
	$status = 'cf7rl-' . $status;
	$statuses = cf7rl_get_payment_statuses();
 ...
 * @return float
 */
function cf7rl_get_earnings_amount($mode = 'live') {
	global $wpdb;
	...
	$earnings = $wpdb->get_results(
		"SELECT SUM(pm2.meta_value) AS amount
		 ...
		 WHERE p.post_type ='cf7rl_payments'
		   AND p.post_status ='cf7rl-completed'
 ...
 */
function cf7rl_is_local_url( $url = '' ) {
		$is_local_url = false;
		...
		if ( ! empty( $url ) && ! empty( $host ) ) {
			...
			$check_tlds = apply_filters( 'cf7rl_validate_tlds', true );
			if ( $check_tlds ) {
				$tlds_to_check = apply_filters( 'cf7rl_url_tlds', array(
					'.dev', '.local', '.test',
				) );
			...
			if ( substr_count( $host, '.' ) > 1 ) {
				$subdomains_to_check = apply_filters( 'cf7rl_url_subdomains', array(
 ...
 * @since 1.8
 */
add_action( 'admin_head', 'cf7rl_show_cf7rl_payments_localhost_notice' );
function cf7rl_show_cf7rl_payments_localhost_notice () {
    global $current_screen;
	
	if ($current_screen->post_type == 'cf7rl_payments') {
		
		$actual_link = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]";
		
		$local = cf7rl_is_local_url($actual_link);
		
		if ($local == true) {
			
			function cf7rl_show_cf7rl_payments_localhost_admin_notice() {
				?>
		...
		add_action( 'admin_notices', 'cf7rl_show_cf7rl_payments_localhost_admin_notice' );
