...
function cf7rl_stripe_connection_status() {
	...
	if ( !isset($stripeConnectionStatus) ) {
		...
		if (!empty($account_id)) {
			$url = cf7rl_STRIPE_CONNECT_ENDPOINT . '?' . http_build_query(
				array(
					'action'		=> 'checkStatus',
					'mode'			=> $mode,
...
function cf7rl_stripe_connection_status_html() {
	$connected = cf7rl_stripe_connection_status();

	if ($connected) {
		$reconnect_mode = $connected['mode'] == 'live' ? 'sandbox' : 'live';
		printf(
			'<div class="notice inline notice-success cf7rl-stripe-connect">
			...
			$connected['email'],
			$connected['mode'],
			cf7rl_stripe_connect_url($reconnect_mode),
			$reconnect_mode,
			cf7rl_stripe_disconnect_url($connected['account_id'], $connected['token'])
		);
	} else {
		printf(
			...
			cf7rl_stripe_connect_url()
		);
	}
}

function cf7rl_stripe_connect_url($mode = false) {
	if ( $mode === false ) {
		$options = cf7rl_free_options();
	...
	$stripe_connect_url = cf7rl_STRIPE_CONNECT_ENDPOINT . '?' . http_build_query(
		array(
			'action'		=> 'connect',
			'mode'			=> $mode,
			'return_url'	=> cf7rl_stripe_connect_tab_url()
		)
	);
...
function cf7rl_stripe_disconnect_url($account_id, $token) {
	$options = cf7rl_free_options();
...
function cf7rl_stripe_connect_tab_url() {
	return add_query_arg(
		array(
			'page'	=> 'cf7rl_admin_table',
			'tab'	=> '5',
			'cf7rl_free_nonce' 	=> wp_create_nonce('cf7rl_free_stripe')
		),
		admin_url('admin.php')
	);
}

add_action('plugins_loaded', 'cf7rl_stripe_connect_completion');
function cf7rl_stripe_connect_completion() {
	if ((empty($_GET['cf7rl_stripe_connect_completion']) && empty($_GET['cf7pp_stripe_connect_completion'])) || empty($_GET['mode']) || empty($_GET['account_id']) || empty($_GET['token'])) return;
	
	if ((!empty($_GET['cf7rl_stripe_connect_completion']) && intval($_GET['cf7rl_stripe_connect_completion']) !== 1) && (!empty($_GET['cf7pp_stripe_connect_completion']) && intval($_GET['cf7pp_stripe_connect_completion']) !== 1)) return;
	
	// nonce check
	if (!isset($_GET['cf7rl_free_nonce']) || !wp_verify_nonce($_GET['cf7rl_free_nonce'], 'cf7rl_free_stripe')) {
	 ...
	 */
	$return_url = apply_filters('cf7rl_stripe_connect_return_url', $return_url);

	wp_redirect($return_url);
	exit;
}

add_action('plugins_loaded', 'cf7rl_stripe_disconnected');
function cf7rl_stripe_disconnected() {
	if ((empty($_GET['cf7rl_stripe_disconnected']) && empty($_GET['cf7pp_stripe_disconnected'])) || empty($_GET['mode']) || empty($_GET['account_id'])) return;
	
	if ((!empty($_GET['cf7rl_stripe_disconnected']) && intval($_GET['cf7rl_stripe_disconnected']) !== 1) && (!empty($_GET['cf7pp_stripe_disconnected']) && intval($_GET['cf7pp_stripe_disconnected']) !== 1)) return;
	 ...
	 * @param $return_url URL to return to.
	 */
	$return_url = apply_filters('cf7rl_stripe_disconnect_return_url', $return_url);
