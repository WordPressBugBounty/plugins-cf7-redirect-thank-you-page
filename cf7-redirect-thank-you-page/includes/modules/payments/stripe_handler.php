 ...
 */
add_action('wpcf7_before_send_mail', 'cf7rl_insert_webhook_data_to_plugin_settings');
function cf7rl_insert_webhook_data_to_plugin_settings() {
	...
	switch ($options['mode_stripe']) {
		case '1':
			if (!empty($options['sec_key_test'])) {
				...
				$options['webhook_data_test'] = cf7rl_maybe_create_stripe_webhook($options['webhook_data_test'], $options['sec_key_test']);
		...
		case '2':
			if (!empty($options['sec_key_live'])) {
				...
				$options['webhook_data_live'] = cf7rl_maybe_create_stripe_webhook($options['webhook_data_live'], $options['sec_key_live']);
 ...
 */
function cf7rl_maybe_create_stripe_webhook($webhook_data, $sk) {
	...
	$url = cf7rl_get_stripe_webhook_url();
 ...
 */
function cf7rl_get_stripe_webhook_url($return = 'str') {
	$options = cf7rl_free_options();
	$mode_stripe = $options['mode_stripe'] == '1' ? 'test' : 'live';

	$namespace = 'stripewebhooks/v1';
	$route = '/cf7rl_' . $mode_stripe;
 ...
 * @since 1.8
 */
add_action('rest_api_init', 'cf7rl_stripe_webhook_listener');
function cf7rl_stripe_webhook_listener() {
	$webhook_url = cf7rl_get_stripe_webhook_url('arr');
    register_rest_route($webhook_url['namespace'], $webhook_url['route'], array(
        'methods' 				=> 'POST',
        'callback' 				=> 'cf7rl_stripe_webhook_handler',
        'permission_callback'	=> 'cf7rl_stripe_webhook_auth'
    ));
}
 ...
 * @return bool
 */
function cf7rl_stripe_webhook_auth() {
 ...
 * @since 1.8
 */
function cf7rl_stripe_webhook_handler() {
	$options = cf7rl_free_options();
	$mode_stripe = $options['mode_stripe'] == '1' ? 'test' : 'live';
	...
	switch ($event->type) {
		case 'checkout.session.completed':
			...
			$transaction_id = $event->data->object->payment_intent;
         	cf7rl_complete_payment($payment_id, $status, $transaction_id);
            break;
 ...
 * @return string or array
 */
function cf7rl_get_stripe_connect_webhook_url($return = 'str') {
	$arg = 'cf7rl_notice';
	$val = 'stripewebhook';
 ...
 * @since 1.8
 */
add_action('plugins_loaded', 'cf7rl_stripe_connect_webhook_listener');
function cf7rl_stripe_connect_webhook_listener() {
	// check if webhook endpoint
	$webhook_url = cf7rl_get_stripe_connect_webhook_url('arr');
	...
	$result = cf7rl_complete_payment($_REQUEST['payment_id'], $_REQUEST['status'], $_REQUEST['transaction_id']);
