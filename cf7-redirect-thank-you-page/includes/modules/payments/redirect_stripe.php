...
function cf7rl_stripe_redirect($post_id,$fid,$return_url,$payment_id) {
	
	$options = cf7rl_free_options();
	
	
	// get variables
	$name = 	sanitize_text_field(get_post_meta($post_id, "_cf7rl_name", true));
	$price = 	sanitize_text_field(get_post_meta($post_id, "_cf7rl_price", true));
	$id = 		sanitize_text_field(get_post_meta($post_id, "_cf7rl_id", true));
	...
	if ($session == '1') {
		$stripe_return	= isset($_COOKIE['cf7rl_stripe_return']) ? sanitize_text_field($_COOKIE['cf7rl_stripe_return']) : NULL;
		$stripe_email	= isset($_COOKIE['cf7rl_stripe_email']) ? sanitize_text_field($_COOKIE['cf7rl_stripe_email']) : NULL;
	} else {		
		$stripe_return 	= isset($_SESSION['cf7rl_stripe_return']) ? sanitize_text_field($_SESSION['cf7rl_stripe_return']) : NULL;
		$stripe_email 	= isset($_SESSION['cf7rl_stripe_email']) ? sanitize_text_field($_SESSION['cf7rl_stripe_email']) : NULL;
	}	
	...
	if (empty($name)) 		{ $name =  __("(No item name)", 'contact-form-7-paypal-add-on'); }
	...
	if (!empty($stripe_sec)) {
		...
		$stripe_connect_url = cf7rl_STRIPE_CONNECT_ENDPOINT . '?' . http_build_query(
			array(
				...
				'cancel_url'			=> $cancel_url,
				'notice_url'			=> cf7rl_get_stripe_connect_webhook_url(),
				'client_reference_id'	=> $payment_id,
